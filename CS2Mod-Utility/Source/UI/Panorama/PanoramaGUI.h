...
(function () {
  ...
  $.DispatchEvent('Activated', $.GetContextPanel().FindChildTraverse("MainMenuNavBarHome"), 'mouse');
        ...
        if (const auto guiPanel = mainMenu.findChildInLayoutFile("OsirisMenuTab")) {
            ...
            hookContext.template make<SkinTab>().init(guiPanel);
            hookContext.template make<MusicKitTab>().init(guiPanel);
        }
    ...
    void updateFromConfig() noexcept
    {
        ...
        hookContext.template make<SkinTab>().updateFromConfig(mainMenu);
        hookContext.template make<MusicKitTab>().updateFromConfig(mainMenu);
    }
